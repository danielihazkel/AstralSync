...
export function buildLifeStoryPrompt(
  ...
  const instructions = [
    ...
    "Each event may carry a \"Timing:\" line giving the traditional time-lords actually in force on that date — the annual profection and its lord, the firdaria period, and zodiacal releasing from the Lot of Fortune. These are computed, not guessed: use them where they illuminate an event, and say when a lord matches a chart theme. Where an event has no Timing line, do not invent one.",
    "You may also relate events qualitatively to well-known slow cycles (the Saturn return around ages 28–30 and 57–59, for instance), but make no precise transit claims beyond the timing given, and treat month- and year-precision dates as approximate.",
